  ...
  if (recommendations.length === 0) {
    return (
      <div className="glass-card p-12 text-center max-w-xl mx-auto space-y-6">
        <Map className="w-16 h-16 text-slate-400 dark:text-slate-600 mx-auto animate-pulse" />
        <h3 className="text-xl font-bold text-slate-900 dark:text-white">No Active Recommendation Path</h3>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white">Upskilling Learning Roadmap</h1>
        ...
        <div className="flex items-center space-x-2 self-start sm:self-center">
          <label className="text-xs font-bold text-slate-700 dark:text-slate-300 whitespace-nowrap">Target Career:</label>
          <select
            value={selectedIdx}
            onChange={(e) => setSelectedIdx(parseInt(e.target.value))}
            className="form-input text-xs py-1.5 px-3 bg-white dark:bg-slate-900 border border-slate-250 dark:border-slate-800 text-slate-900 dark:text-slate-205 cursor-pointer"
          >
      ...
      <div className="glass-card p-6 space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300">Overall Progress</span>
            <span className="text-xs font-black text-primary">{progressPercent}%</span>
          </div>
          <span className="text-[10px] text-slate-650 dark:text-slate-400 font-bold uppercase">{completedCount} of {allMilestones.length} Milestones Achieved</span>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="glass-card p-6 space-y-6 relative before:absolute before:left-7 before:top-8 before:bottom-8 before:w-[2px] before:bg-slate-200 dark:before:bg-slate-800">
            ...
            <div className="relative pl-12 space-y-4">
              ...
              <div>
                <span className="text-[9px] font-black uppercase px-2 py-0.5 bg-primary/10 text-primary-dark dark:text-primary-light rounded-md">Day 1 - 30</span>
                <h3 className="text-sm font-extrabold text-slate-900 dark:text-slate-200 mt-1">{roadmap.plan_30.title}</h3>
              </div>
                ...
                {roadmap.plan_30.milestones.map((m: string) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left p-3 rounded-xl border flex items-start space-x-3 transition-colors cursor-pointer ${
                        isDone 
                          ? 'bg-emerald-500/5 border-emerald-500/20 text-slate-500 dark:text-slate-400' 
                          : 'bg-white/40 dark:bg-slate-900/40 border-slate-200 dark:border-slate-800 text-slate-850 dark:text-slate-200 hover:border-slate-300 dark:hover:border-slate-700'
                      }`}
                    >
                      {isDone ? (
                        <CheckSquare className="w-4.5 h-4.5 text-emerald-600 dark:text-emerald-550 mt-0.5 flex-shrink-0" />
                      ) : (
                        <Square className="w-4.5 h-4.5 text-slate-600 dark:text-slate-400 mt-0.5 flex-shrink-0" />
                      )}
            ...
            <div className="relative pl-12 space-y-4">
              ...
              <div>
                ...
                <h3 className="text-sm font-extrabold text-slate-900 dark:text-slate-200 mt-1">{roadmap.plan_60.title}</h3>
              </div>
            ...
            <div className="relative pl-12 space-y-4">
              ...
              <div>
                ...
                <h3 className="text-sm font-extrabold text-slate-900 dark:text-slate-200 mt-1">{roadmap.plan_90.title}</h3>
              </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="glass-card p-6 space-y-4">
            <h3 className="font-bold text-sm text-slate-900 dark:text-slate-200 border-b border-slate-200 dark:border-slate-800 pb-2 flex items-center space-x-1.5">
            ...
            <div className="space-y-3">
              {activeRec.courses.map((course: any) => (
                <div key={course.name} className="p-4 bg-slate-50/50 dark:bg-slate-900/30 border border-slate-200 dark:border-slate-800/60 rounded-xl space-y-2">
                  <span className="text-[8px] uppercase font-bold px-1.5 py-0.5 bg-accent/10 text-accent rounded">
                    {course.provider}
                  </span>
                  <h5 className="font-bold text-xs leading-tight text-slate-900 dark:text-white">{course.name}</h5>
                  <p className="text-[9px] text-slate-600 dark:text-slate-400">Category Focus: {course.category}</p>
                  {course.url && (
                    <a
                      href={course.url}
                      target="_blank"
